...
let express = require("express"),
	...
	{ exploitPatch, Utils, web, checkKeys } = require("../utils"),
	{ remove } = exploitPatch,
	fs = require("fs"),
	zlib = require("zlib");
router.all("/database/accounts/backupGJAccountNew(.php)?", web.gdMiddleware, (req, res) => {
	console.log('PUDJPUDJpidorPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJPUDJ')
	if (!checkKeys(req.body, ['userName', 'password', 'saveData'])) return res.send('-1');
	const vars = req.body,
...
router.all("/database/accounts/syncGJAccount(20)?(New)?(.php)?", web.gdMiddleware, (req, res) => {
	if (!checkKeys(req.body, ['userName', 'password'])) return res.send('-1');
	let vars = req.body,
