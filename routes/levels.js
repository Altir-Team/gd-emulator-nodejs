...
let express = require("express"),
	...
	{ exploitPatch, Utils, XOR, GJPCheck, web, checkKeys } = require("../utils"),
	{ remove, numcolon, numbers } = exploitPatch,
	fs = require("fs"),
	zlib = require("zlib"),
	fuse = require("fuse.js"),
	moment = require("moment");
router.post("/uploadGJLevel21(.php)?", web.gdMiddleware, (req, res) => {
	let vars = req.body;
	if (!checkKeys(vars, ['gameVersion', 'binaryVersion', 'gdw', 'udid', 'uuid', 'userName', 'levelID', 'levelName', 'levelDesc', 'levelVersion', 'levelLength', 'audioTrack', 'auto', 'password', 'original', 'twoPlayer', 'songID', 'objects', 'coins', 'requestedStars', 'unlisted', 'wt', 'wt2', 'ldm', 'extraString', 'seed', 'seed2', 'levelString', 'levelInfo'])) return res.send('-1');
	if (!Utils.isValidID(vars.accountID)) return res.send("-1");
...
router.post("/getGJLevels(21|20)?(.php)?", web.gdMiddleware, (req, res) => {
	console.log(checkKeys(req.body, ['gameVersion', 'binaryVersion', 'gdw', 'type', 'str', 'diff', 'len', 'page', 'total', 'uncompleted', 'onlyCompleted', 'featured', 'original', 'twoPlayer', 'coins', 'epic']))
	if (!checkKeys(req.body, ['gameVersion', 'binaryVersion', 'gdw', 'type', 'str', 'diff', 'len', 'page', 'total', 'uncompleted', 'onlyCompleted', 'featured', 'original', 'twoPlayer', 'coins', 'epic'])) return res.send('-1');
	let vars = req.body,
		lvlString = [],
		userString = [],
		songsString = [],
	...
	if (vars.gauntlet) {
		...
		let gaut = global.database.prepare("SELECT * FROM gauntlets WHERE id = ?").all(gauntlet);
		gaut = gaut[0];
	...
	if (type == 0 || type == 15) {
		order = "likes DESC";
		if (str.length) {
			if(Utils.isNumeric(str)) { params = [`levelID = ${String(str)}`]; search = false; }
			else search = true;
		}
	}
	...
	if (type == 5) params.push(`userID = '${String(str)}'`);
	...
	result.map(x => {
		if (x.levelID) {
			...
			userString.push(`${Utils.getUserString(String(x.userID).split(".")[0])}`);
		}
	});
...
router.post("/downloadGJLevel(19|20|21|22)?(.php)?", web.gdMiddleware, (req, res) => {
	if (!checkKeys(req.body, ['gameVersion', 'binaryVersion', 'gdw', 'levelID'])) return res.send('-1');
	let vars = req.body,
	...
	switch (levelID) {
	case -1:
		let dailyLevel = global.database.prepare("SELECT * FROM dailyFeatures WHERE timestamp < ? AND type = 0 ORDER BY DESC LIMIT 1").all(parseInt(Date.now() / 1000)); // eslint-disable-line
		dailyLevel = dailyLevel[0];
		levelID = dailyLevel.levelID;
		feaID = dailyLevel.feaID + 100001;
		daily = 1;
		break;
	case -2:
		let weeklyLevel = global.database.prepare("SELECT * FROM dailyFeatures WHERE timestamp < ? AND type = 1 ORDER BY DESC LIMIT 1").all(parseInt(Date.now() / 1000)); // eslint-disable-line
		weeklyLevel = weeklyLevel[0];
		levelID = weeklyLevel.levelID;
		feaID = weeklyLevel.feaID + 100001;
		daily = 1;
		break;
	}
	...
	if(levels.length) {
		...
		let smth = `${String(lvl.userID)},${lvl.starStars},${lvl.starDemon},${lvl.levelID},${lvl.starCoins},${lvl.starFeatured},${password},${feaID}`;
